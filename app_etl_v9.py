...
class ETLApp:
    def __init__(self, master):
        self.master = master
        master.title("ETL")
        master.geometry("1000x600")  # Tamanho maior para melhor visualização
        ...
        self.df = None  # DataFrame para ETL
        self.transformation_widgets = []  # [(col, trans_var, conv_var, param1_var, param2_var)]

        # Campo para digitar nome da tabela
        ...
        # Frame para transformações
        self.transformation_frame = tk.Frame(master)
        self.transformation_frame.pack(pady=10, fill='both', expand=True)
    ...
    def generate_transformation_fields(self):
        ...
        for col in self.df.columns:
            ...
            def on_conv_change(event=None, param_frame=param_frame, conv_var=conv_var, param1_var=param1_var, param2_var=param2_var):
                ...
                if sel == "VARCHAR":
                    tk.Label(param_frame, text="Len:").pack(side='left')
                    tk.Entry(param_frame, textvariable=param1_var, width=5).pack(side='left')

                elif sel == "DECIMAL":
                    tk.Label(param_frame, text="Precision:").pack(side='left')
                    tk.Entry(param_frame, textvariable=param1_var, width=5).pack(side='left')

                    tk.Label(param_frame, text="Scale:").pack(side='left')
    ...
    def update_treeview(self):
        ...
        for col in self.df.columns:
            ...
            self.tree.column(col, width=100, anchor='w')  # Define largura padrão
        ...
        for col, _, conv_var, param1_var, param2_var in self.transformation_widgets:
            ...
            try:
                if conv in ["Texto", "STRING"]:
                    ...
                    precision = int(param1) if param1.isdigit() else None
